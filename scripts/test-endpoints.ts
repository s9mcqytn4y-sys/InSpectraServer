...
async function runTests() {
  ...
  if (!material || !part || !defect) {
    console.log("❌ Master data belum lengkap. Harap jalankan seeder terlebih dahulu.");
    process.exit(1);
  }
  ...
  try {
    ...
    if (laporanDetailId) {
      await prisma.e_laporan_produksi_detail.delete({ where: { id: laporanDetailId } }).catch(() => {});
      console.log("🗑️ Laporan Detail dihapus.");
    }
    if (laporanId) {
      await prisma.e_laporan_produksi.delete({ where: { id: laporanId } }).catch(() => {});
      console.log("🗑️ Laporan dihapus.");
    }

    if (defectId) {
      await prisma.e_defect_checksheet.delete({ where: { id: defectId } }).catch(() => {});
      console.log("🗑️ Checksheet Defect dihapus.");
    }
    if (itemChecksheetId) {
      await prisma.e_item_checksheet.delete({ where: { id: itemChecksheetId } }).catch(() => {});
      console.log("🗑️ Checksheet Item dihapus.");
    }

    if (cuttingBatchId) {
      await prisma.cuttingBatch.delete({ where: { id: cuttingBatchId } }).catch(() => {});
      console.log("🗑️ Cutting Batch dihapus.");
    }
    
    if (sessionChecksheetId) {
      await prisma.checksheetSession.delete({ where: { id: sessionChecksheetId } }).catch(() => {});
